...
class Hangman():
    def __init__(self, words):
        ...
        Selects the secret word for the game randomly
        """
    ...
    def hangman_display(self):
        ...
        print(HANGMANPICS[len(self._missed_letters)])

        print('Missed letters:', end=' ')
        for letter in self._missed_letters:
            print(letter, end=' ')
        print("\n")
        #Censors the word so they don't see secret
    ...
    def word(self, word):
        '''takes the word and checks if it matches secret word'''
        if word == self._secret_word:
            print("Congratulations you got it!..")
            self._check_win()
            if self._check_win() == True:
                self._game_is_done = True
        else:
            print("Try again, that's not it :)")
    ...
    def _get_guess(self, already_guessed):
        ...
        """ 
        #This code makes sure the user doesn't enter more than one character and only letter
        while True:
            guess = input('Guess one letter or the entire word: ').lower()
            if len(guess) > 1:
                self.word(guess)
    ...
    def _check_win(self):
        ...
        for i in range(len(self._secret_word)):
            if self._secret_word[i] not in self._correct_letters:
                return False
        #Guessed the word correctly and it shows the word it has chosen
    ...
    def _check_lost(self):
        ...
        if len(self._missed_letters) == len(HANGMANPICS) - 1:
            ...
            #This is when the user runs out of guesses and loses the game
    ...
    def run(self):
        """Initializes the game play and coordinates the game activities."""

        print('H A N G M A N\n')
        print("Guess a letter or type in the word you think it is...The theme is animals")
...
def play_again():
   #Asks if the user would like to play the game again if they have lost or won
